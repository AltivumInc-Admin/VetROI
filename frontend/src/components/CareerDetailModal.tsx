...
import '../styles/CareerDetailModal.css'
...
export const CareerDetailModal: React.FC<CareerDetailModalProps> = ({
  ...
  useEffect(() => {
    ...
    // Prevent wheel event from propagating to body when modal is open
    const handleWheel = (e: WheelEvent) => {
      const modalCard = document.querySelector('.career-detail-modal .career-detail-card')
      if (modalCard && modalCard.contains(e.target as Node)) {
        e.stopPropagation()
      }
    }

    if (isOpen) {
      // Save current body position and lock scrolling
  ...
  return (
    <>
      ...
      <div className="career-detail-modal">
        ...
        {/* This is the EXACT same content as the expanded card */}
        <div className="career-detail-card expanded in-modal">
          {/* Header Section */}
